 ...
 * cannot proceed past this screen; on success we mirror the number into
 * public.users.phone_number and stamp phone_verified_at, then advance.
 */
export function PhoneVerifyScreen() {
  ...
  // so they'd otherwise re-enter PhoneVerify on relaunch. Skip when verify
  // just succeeded in this same mount — onVerify already handles the navigate.
  useEffect(() => {
  ...
  // Auto-focus the code input when we transition steps.
  useEffect(() => {
    if (step === 'enter_code') {
  ...
  const onVerify = async () => {
    ...
    try {
      ...
      // Hydrate from the server-returned row rather than the values we sent —
      // the user_phone_backfill trigger may rewrite phone_number, and the
      // server-stamped phone_verified_at avoids client clock drift.
          ...
          {step === 'enter_phone' ? (
            <>
              ...
              <Text style={styles.subhead}>
                We'll text a 6-digit code. Friends can find you and tag you in
                meals — we won't text you about anything else.
              </Text>
            ...
            <>
              ...
              <Text style={styles.subhead}>
                Sent to {profile?.phone_number ?? e164}.
                {' '}
